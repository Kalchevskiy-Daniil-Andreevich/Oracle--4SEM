...
--7
ALTER TABLE T_RANGE MERGE PARTITIONS P1, P2 INTO PARTITION P5; --объединение двух разделов
SELECT * FROM T_RANGE PARTITION(P5);

--8
ALTER TABLE T_INTERVAL SPLIT PARTITION P2 AT (TO_DATE('25-08-2014', 'DD-MM-YYYY')) --разделение
...
ALTER TABLE T_LIST EXCHANGE PARTITION P2 WITH TABLE T_LIST_NEW WITHOUT VALIDATION; --обмен местами содержимого 

SELECT * FROM T_LIST PARTITION(P2);
SELECT * FROM T_LIST_NEW;

DROP TABLE T_LIST_NEW;

--10
SELECT TABLE_NAME, PARTITIONING_TYPE --Список всех секционированных таблиц
FROM USER_PART_TABLES;

SELECT PARTITION_NAME, HIGH_VALUE --Список всех секций таблицы T_RANGE
FROM USER_TAB_PARTITIONS
WHERE TABLE_NAME = 'T_RANGE';

SELECT * FROM T_RANGE WHERE ID < 100; --Список всех значений из секции P1 таблицы T_RANGE по ссылке:
